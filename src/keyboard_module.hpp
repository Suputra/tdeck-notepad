...
// --- BLE HID Keyboard Mode Handler ---

static uint8_t btKeyboardExtraModifiers() {
    // Treat ALT as Command on phones for app/shortcut key combos.
    return alt_mode ? BT_HID_MOD_LGUI : 0;
}

bool handleKeyboardModeKeyPress(int event_code) {
    int key_num = (event_code & 0x7F);
    int idx = key_num - 1;
    int row = idx / KEYPAD_COLS;
    int col_raw = idx % KEYPAD_COLS;
    ...
    if (IS_LSHIFT(row, col_rev)) { shift_held = !shift_held; return true; }
    if (IS_RSHIFT(row, col_rev)) { nav_mode = !nav_mode; return true; }
    if (IS_SYM(row, col_rev))    { sym_mode = true; return true; }
    if (IS_ALT(row, col_rev))    { alt_mode = !alt_mode; return true; }
    if (IS_MIC(row, col_rev))    {
        if (sym_mode) {
            sym_mode = false;
            btTypeChar('0', btKeyboardExtraModifiers());
            if (shift_held) shift_held = false;
            if (alt_mode) alt_mode = false;
            return false;
        }
        mic_last_press = millis(); // single press opens command mode via loop timeout
        return false;
    }
    if (IS_DEAD(row, col_rev))   { return false; }
    ...
    if (nav_mode) {
        char base = keymap_lower[row][col_rev];
        uint8_t mods = btKeyboardExtraModifiers();
        if (base == 'w') { btSendUsage(BT_HID_KEY_UP, mods); return false; }
        if (base == 's') { btSendUsage(BT_HID_KEY_DOWN, mods); return false; }
        if (base == 'd') { btSendUsage(BT_HID_KEY_RIGHT, mods); return false; }
        if (base == 'a') { btSendUsage(BT_HID_KEY_LEFT, mods); return false; }
        if (base == '\b') { btSendUsage(BT_HID_KEY_BACKSPACE, mods); return false; }
        return false;
    }
    ...
    bool sent = false;
    if (c == '\b') {
        sent = btSendUsage(BT_HID_KEY_BACKSPACE, mods);
    } else if (c == '\n') {
        sent = btSendUsage(BT_HID_KEY_ENTER, mods);
    } else if (c == '\t') {
        sent = btSendUsage(BT_HID_KEY_TAB, mods);
    } else if (c >= ' ' && c <= '~') {
        sent = btTypeChar(c, mods);
    }
